  ...
  Eigen::Matrix<float, Eigen::Dynamic, 3, Eigen::RowMajor> &N)
{
    ...
    for (size_t s = 0; s < shapes.size(); ++s) {
        ...
        for (size_t f = 0; f < mesh.num_face_vertices.size(); ++f) {
            ...
            if (fv != 3) {
                std::cerr << "Error: OBJ is not purely triangular (face with "
    ...
    for (size_t s = 0; s < shapes.size(); ++s) {
        ...
        for (size_t f = 0; f < mesh.num_face_vertices.size(); ++f) {
            size_t fv = static_cast<size_t>(mesh.num_face_vertices[f]); // 3

            for (size_t v = 0; v < fv; ++v) {
                const tinyobj::index_t idx = mesh.indices[index_offset + v];

                GLuint this_vi = static_cast<GLuint>(vert_idx);

                const int v_idx = idx.vertex_index;
                V(vert_idx, 0) = attrib.vertices[3 * v_idx + 0];
                V(vert_idx, 1) = attrib.vertices[3 * v_idx + 1];
                V(vert_idx, 2) = attrib.vertices[3 * v_idx + 2];

                if (idx.texcoord_index >= 0) {
                    const int t_idx = idx.texcoord_index;
                    UV(vert_idx, 0) = attrib.texcoords[2 * t_idx + 0];
                    UV(vert_idx, 1) = attrib.texcoords[2 * t_idx + 1];
                }
                else {
                    UV(vert_idx, 0) = 0.0f;
                    UV(vert_idx, 1) = 0.0f;
                ...
                if (idx.normal_index >= 0) {
                    const int n_idx = idx.normal_index;
                    N(vert_idx, 0) = attrib.normals[3 * n_idx + 0];
                    N(vert_idx, 1) = attrib.normals[3 * n_idx + 1];
                    N(vert_idx, 2) = attrib.normals[3 * n_idx + 2];
                }
                else {
                    N(vert_idx, 0) = 0.0f;
                    N(vert_idx, 1) = 0.0f;
                ...
                F(tri_idx, static_cast<int>(v)) = this_vi;
...
Usage:
  ...
  if (!load_obj_to_eigen(argv[2], V, F, UV, N)) {
    std::cerr<<"Failed to open OBJ file. Please use the provided OBJ file."<<std::endl;
    return EXIT_FAILURE;
  }
  mesh_to_vao(V, F, UV, N, VAO);
